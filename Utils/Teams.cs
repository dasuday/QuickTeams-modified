...
namespace QuickTeams.Utils
{
    public class Teams
    {
        ...
        public static void DownloadTeam(string sourceTeamId,string aadAccessToken)
        {
            ...
            Files.CreateArchivePath(Path.Combine(channelDownloadFolder,dirName));

            var teamDetails = Teams.GetTeamDetails(sourceTeamId, aadAccessToken);
            Files.CreateArchiveFile(teamDetails, "teamSettings", Path.Combine(channelDownloadFolder,dirName));

            var groupSettingDetails = Groups.GetGroupDetails("",sourceTeamId, aadAccessToken);
            Files.CreateArchiveFile(groupSettingDetails, "groupSettings", Path.Combine(channelDownloadFolder,dirName));

            var groupMemberDetails = Groups.GetGroupDetails("/members",sourceTeamId, aadAccessToken);
            Files.CreateArchiveFile(groupMemberDetails, "groupMembers", Path.Combine(channelDownloadFolder,dirName));

            var groupOwnerDetails = Groups.GetGroupDetails("/owners",sourceTeamId, aadAccessToken);
            Files.CreateArchiveFile(groupOwnerDetails, "groupOwners", Path.Combine(channelDownloadFolder,dirName));
            ...
            for (int i = 0; i < msTeamsChannels.value.Count; i++)
            {
                ...
                Channels.DownloadChannelMessagesAndReplies("",channelDownloadFolder, msTeamsChannels.value[i], msTeamsChannels.value[i].displayName, sourceTeamId, aadAccessToken);
            }
