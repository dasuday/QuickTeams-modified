...
namespace QuickTeams.Utils
{
    public class Channels
    {
        ...
        public static string SelectChannel(string teamId, string aadAccessToken)
        {
            ...
            Console.WriteLine("Dingbat");
            Console.WriteLine(msTeamsChannels.ToString());
            ...
            string selectedChannelName = selectedChannel.displayName;
            ...
            if (Console.ReadLine().StartsWith("y", StringComparison.InvariantCultureIgnoreCase))
            {
                ...
                DownloadChannelMessagesAndReplies("",channelDownloadFolder, selectedChannel, selectedChannelName, teamId, aadAccessToken);
            }
        ...
        public static void DownloadChannelMessagesAndReplies(string parentMessageId, string channelDownloadFolder, MsTeamsChannels.ChannelObject selectedChannel, string selectedChannelName, string teamId, string aadAccessToken)
        {
            string dirName = "GPAST - DL " + System.DateTime.Today.ToString().Replace("/", "-").Replace(":", "-");
            //Console.WriteLine(dirName);
            //var archivePath = Path.Combine(channelDownloadFolder, dirName, selectedChannel.id.ToString().Remove(2,1));
            var archivePath = Path.Combine(channelDownloadFolder, dirName, selectedChannelName);
            ...
            while (nextUrl != "")
            {
                ...
                for(int i=0;i < channelArchiveJson.value.Count; i++)
                {
                    ...
                    if(parentMessageId == "")
                    {
                        ...
                        DownloadChannelMessagesAndReplies(messageId,channelDownloadFolder,selectedChannel, selectedChannelName, teamId, aadAccessToken);
                    }
                    else
                    {
        ...
        public static MsTeamsChannels.Channels GetChannels(string teamId, string aadAccessToken)
        {
            ...
            var httpResponseMessage =
                    Helpers.httpClient.GetAsync(O365.MsGraphBetaEndpoint + "teams/" + teamId + "/channels").Result;
            Console.WriteLine("Confirming Teamid: " + teamId);
            Console.WriteLine("Access Token:" + aadAccessToken);
